...
const EASE = cubicBezier(0.24, 0.43, 0.15, 0.97);
const EASE_REVERSED = cubicBezier(0.97, 0.15, 0.43, 0.24);
...
export default function Header({
  ...
  const animateLargeHeader = (showMenu: boolean) => {
    ...
    const duration =
      Math.abs((clipProgress.get() - end) / MAX_VALUE) * DURATION;
  ...
  return (
    <header ref={ref} className="max-h-0 sticky top-0 z-10">
      <div
        ref={childRef}
      ...
      >
        <div className="w-full h-[60px] md:h-[120px] flex items-center justify-between gap-10">
          <span className="text-xl lg:text-4xl leading-[1rem] lg:leading-none font-light uppercase tracking-[.5rem] lg:tracking-[.7rem]">
            {"Elementis".split("").map((letter, index) => (
              ...
              {[
                ...
                <li key={title} className="overflow-hidden">
                  ...
                  >
                    <a
                      href="#"
                      className="block relative before:absolute before:top-full before:left-0 before:w-full before:h-[1px] before:bg-[var(--text)] before:transition-transform before:duration-500 before:ease-expo before:scale-x-0 before:origin-right hover:before:origin-left hover:before:scale-x-100"
                    >
          ...
          <div className="flex gap-8 items-center">
            <motion.button
              ...
              className="relative hiden md:flex items-center justify-center lg:w-[136px] h-10 lg:h-14 px-4 lg:px-0 border border-[var(--border)]"
            >
