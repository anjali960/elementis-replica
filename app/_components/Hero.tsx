...
export default function Hero({
  ...
  const gradientStyling = useTransform(scrollY, () => {
    if (scrollY.get() > windowHeight)
      return "linear-gradient(var(--main-background) 0 100%)";

    const gradient: string[] = [];

    for (let i = 0; i < MAX_GRADIENT_BREAK; i++) {
      const scrollYPercentage = Math.min(
        (scrollY.get() + 350) / windowHeight,
        1
      );
      ...
      gradient[
        i
      ] = `rgba(0, 0, 0, 1) ${start}% ${end1}%, rgba(0, 0, 0, 0) ${end1}% ${end}%`;
    }
  ...
  return (
    <section className="group overflow-x-clip">
      <div className="absolute top-0 left-0 before:absolute before:w-full before:h-full before:bg-black/30">
        ...
        ></video>
        <motion.div
          style={{ maskImage: gradientStyling }}
          className="bg-[var(--main-color)] absolute top-0 left-0 w-full h-[calc(100vh_+_50px)]"
        ></motion.div>
      </div>
        ...
        <div className="flex-1 flex items-end overflow-hidden">
          ...
          >
            <TranslatingTitle
              ...
              textArray={["Wellness", "Innovation", "Nature", "Community"]}
            />
          </motion.div>
        </div>
