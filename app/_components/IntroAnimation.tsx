...
const EASE = [0.24, 0.43, 0.15, 0.97];
const TEXT = "Elementis";
  ...
  useEffect(() => {
    ...
    if (!useDifferentAnimation) {
      ...
      animationControls = animate(
        scope.current,
        { backgroundColor: "var(--main-color)", color: "var(--text-color)" },
        {
          duration: 0.5,
  ...
  return (
    <AnimatePresence>
      {!hide ? (
        ...
        >
          <div className="flex gap-4 w-full h-full items-center justify-center font-light text-4xl uppercase">
            {TEXT.split("").map((letter, index) => (
              <span
                key={index}
                className="font-inter inline-block overflow-hidden"
              >
                <motion.span
                  initial={{ x: useDifferentAnimation ? "0%" : "100%" }}
                  animate={{ x: useDifferentAnimation ? "-105%" : "0%" }}
                  onAnimationComplete={() =>
                    index === TEXT.length - 1 && setUseDifferentAnimation(true)
                  }
                  transition={{
                    duration: 1.5,
                    ease: EASE,
                    delay: useDifferentAnimation
                      ? (index + 10) / 25
                      : index / 25,
                  }}
